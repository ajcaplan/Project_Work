...
def plot_bes_locs(shot, apdpos, timepoint, equilib_R, equilib_Z, equilib_psi):
...
def plot_bes_fluct_spectrum(
        shot, bes_time, fluct_data, column, timeslice, fname):
    ...
    plt.title(('shot' + str(shot) + ' col ' + str(column) + ', times ' + 
        ...
        fontsize=32)
    plt.savefig(('./Plots/shot' + str(shot) + '_col' + str(column) + '_spectrum_' + 
                 str(fname) + '.png'), format='png', transparent=True)
    plt.close()

def get_channel_fft( # A. Caplan
        shot, bes_time, fluct_data, ch, timeslice, fname):
    idx1 = (np.abs(bes_time - timeslice[0])).argmin()
    idx2 = (np.abs(bes_time - timeslice[1])).argmin()
...
def plot_bes_fluctuations(
        shot, bes_time, fluct_data, timeslice, channels, fname, threshold=None):
    ...
    plt.savefig(('./Plots/shot' + str(shot) + '_BES_fluctuations_' + str(fname) + 
...
def plot_with_flux_surfaces(
        apdpos, equilib_R, equilib_Z, equilib_psi, quantity, title, 
